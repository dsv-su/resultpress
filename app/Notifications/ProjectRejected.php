...
class ProjectRejected extends Notification
{
    ...
    public function toMail($notifiable)
    {
        return (new MailMessage)
            ->subject(sprintf('Project `%s` Rejected', $this->project->name))
            ->greeting(sprintf('Hello %s!', $notifiable->name))
            ->line(sprintf('Your project `%s` has been rejected.', $this->project->name))
    ...
    {
        return [
            'message' => sprintf('Project `%s` has been returned for revision.', $this->project->name),
